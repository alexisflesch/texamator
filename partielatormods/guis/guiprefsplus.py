...
def updateUi(self, MyHighlighter):
    ...
    #Signals and slots
    self.ui_prefs.pushButton_parcourir_tex_path.clicked.connect(self.parcourir_tex_path)
    ...
    self.ui_prefs.comboBox_compile.currentIndexChanged[str].connect(self.comboCompile)
    ...
    self.ui_prefs.pushButton_delete_compile_config.clicked.connect(self.delete_compile_config)
    self.ui_prefs.pushButton_newconfig.clicked.connect(self.add_generate_config)
    self.ui_prefs.pushButton_new_compile_config.clicked.connect(self.add_compile_config)
...
def close_prefs(self, res):
    ...
    if res:
        ...
        self.settings["tex_path"] = str(self.ui_prefs.lineEdit_ex_folder.text())
        self.settings["save_location"] = str(self.ui_prefs.lineEdit_save_folder.text())
        self.settings["file_viewer"] = str(self.ui_prefs.lineEdit_dvi_viewer.text())
...
def delete_compile_config(self):
    ...
    self.ui_delete.label.setText(_translate("Form", "You are about to delete the compile sequence : ") + text)
...
def add_generate_config(self):
    ...
    if res and newkey and not newkey in list(self.new_preamblesPostambles.keys()):
        ...
        self.new_preamblesPostambles[newkey][0] = "\\documentclass{article}\n\n\\begin{document}\n\n\n\n\n%Your exercises will be written after these lines"
        self.new_preamblesPostambles[newkey][1] = "\n\n%Your exercises will appear before these lines\n\\end{document}"
...
def combo(self, text):
    ...
    self.new_preamblesPostambles[self.last_generate][0] = str(self.ui_prefs.textEdit_generate.toPlainText())
    self.new_preamblesPostambles[self.last_generate][1] = str(self.ui_prefs.textEdit_generate_footer.toPlainText())
    a = str(text)
    self.ui_prefs.textEdit_generate.setText(self.new_preamblesPostambles[a][0])
    self.ui_prefs.textEdit_generate_footer.setText(self.new_preamblesPostambles[a][1])
    self.last_generate = a
    self.ui_prefs.pushButton_delete.setEnabled(True)


def parcourir_tex_path(self):
    dirName = QtWidgets.QFileDialog.getExistingDirectory(self.Dialog_prefs,_translate("Form", "Pick a folder"), self.settings["tex_path"])
    if dirName:
        self.ui_prefs.lineEdit_ex_folder.setText(dirName)

def parcourir_sav(self):
    dirName = QtWidgets.QFileDialog.getExistingDirectory(self.Dialog_prefs,_translate("Form", "Pick a folder"), self.settings["save_location"])
    if dirName:
...
def prefs_delete(self):
    ...
    self.ui_delete.label.setText(_translate("Form", "You are about to delete the config : ") + text)
    res = Dialog_delete.exec_()
    if res:
        self.ui_prefs.comboBox_gen.removeItem(i)
        del self.new_preamblesPostambles[text]
        home_dir = os.path.expanduser("~")
        os.remove(os.path.join(home_dir,".texamator","preambles.and.postambles",text+".preamble.tex"))
        os.remove(os.path.join(home_dir,".texamator","preambles.and.postambles",text+".postamble.tex"))

...
class MonApplication(Ui_MainWindow):
    ...
    def search(self):
        ...
        string = self.lineEdit_search.text()
        string = string.lower()
        # to allow a search with accents of the form \'e instead of é (see crazyparser.py)
        string2 = power_rangers(string)
            ...
            for root, dirs, files in self.treeTeX:
                ...
                for feuille_dexo in files:
                    ...
                    if liste_dexo:
                        ...
                        fichiers_a_ajouter.append(
                            (feuille_dexo[0], liste_dexo))
                ...
                if dossiers_a_ajouter or fichiers_a_ajouter:
                    self.treeTeX2.append(
                        (root, dossiers_a_ajouter, fichiers_a_ajouter))
            self.treeTeX.reverse()
    ...
    def fillTheTree(self, someTree, nb_ex):
        ...
        if nb_ex == 0:
            self.treeWidget.setHeaderLabel(
                _translate("Dialog", "No exercise found"))
        elif nb_ex == 1:
            self.treeWidget.setHeaderLabel(
                _translate("Dialog", "1 exercise found"))
        else:
            self.treeWidget.setHeaderLabel(
                str(nb_ex) + " " + _translate("Dialog", "exercises found"))
        ...
        if someTree:
            ...
            folderIcon.addPixmap(QtGui.QPixmap(
                _fromUtf8(":/all/icones/folder.png")), QtGui.QIcon.Normal, QtGui.QIcon.Off)
            fileIcon = QtGui.QIcon()
            fileIcon.addPixmap(QtGui.QPixmap(
                _fromUtf8(":/all/icones/tex.png")), QtGui.QIcon.Normal, QtGui.QIcon.Off)

            for fullpath, b, c in someTree:
                nom = fullpath.split("/")[-1]
                parent = os.path.split(fullpath)[0]
                itemDict[fullpath] = QtWidgets.QTreeWidgetItem(
                    itemDict[parent])
                ...
                for i in c:  # TeX file
                    ...
                    itemDict[nom_element] = QtWidgets.QTreeWidgetItem(
                        itemDict[fullpath])
                    ...
                    for j in i[1]:  # Exercises
                        ...
                        itemDict[nom_cp_exo] = QtWidgets.QTreeWidgetItem(
                            itemDict[nom_element])
                        itemDict[nom_cp_exo].setText(0, nom_exo)
                        itemDict[nom_cp_exo].enonce = j
                        itemDict[nom_cp_exo].titre = nom_tex + \
                            " - ex " + str(compteur_exos)
                        itemDict[nom_cp_exo].filename = os.path.join(
                            fullpath, i[0])
    ...
    def show_preview(self, enonce, inside=1):
        ...
        if inside:
            a.write(
                self.preamblesPostambles[self.settings["preferred preamble"]][0])
            if "\\begin{document}" not in self.preamblesPostambles[self.settings["preferred preamble"]][0]:
                a.write("\n\\begin{document}\n")
            a.write(enonce)
            a.write(
                self.preamblesPostambles[self.settings["preferred preamble"]][1])
            a.write("\n\\end{document}")
        else:
            a.write(enonce)
        a.close()
    ...
    def show_preview_outside(self, enonce):
        if self.show_preview(enonce, 0):
            ...
            os.system(self.settings["file_viewer"] +
                      " /tmp/texamator/export.pdf &")
    ...
    def gouptable(self):
        ...
        tuple_selected = sorted(tuple_selected, key=lambda tup: tup[0])
        if tuple_selected:
    ...
    def populate_compile(self):
        ...
        for key in sorted(self.compile_seq):
            ...
            if self.settings["preferred compile sequence"] == key:
                icon.addPixmap(QtGui.QPixmap(":/all/icones/apply.png"),
    ...
    def tableSelectionChanged(self):
        ...
        if not rows:  # no selection
            ...
            if self.settings['AMC'] == 'False':
                ...
                # one of the element is at the bottom of the table
                if rows[-1] == self.tableWidget.rowCount()-1:
                    self.button_down.setEnabled(False)
                else:
    ...
    def removefromcompileseq(self):
        self.ui_prefs.listWidget_comp.takeItem(
            self.ui_prefs.listWidget_comp.currentRow())
    ...
    def editExerciceTableWidget(self):
        ...
        if self.tableWidget.selectedItems()[0].column():
            ...
            self.ui_edit.textEdit.setText(
                self.tableWidget.currentItem().enonce)
            ...
            if res:
                ...
                if self.settings['AMC'] == 'True':
                    ...
                    item.setText(self.findAMCGroup(
                        self.tableWidget.currentItem().enonce))
    ...
    def itemChangedTable(self, item):
        ...
        if self.settings['AMC'] == 'True' and not item.column():
            ...
            if not itemAMC:
                ...
                itemAMC.setFlags(QtCore.Qt.ItemIsSelectable |
            ...
            if item.text():
                self.changeElement(self.tableWidget.item(
                    item.row(), 0), item.text())
            else:
                item.setText(self.findAMCGroup(
                    self.tableWidget.item(item.row(), 0).enonce))
    ...
    def copyToClipboardFromTable(self):
        ...
        tuple_selected = [(self.tableWidget.row(item), item)
                          for item in self.tableWidget.selectedItems()]
        # sort by position in the table
        tuple_selected = sorted(tuple_selected, key=lambda tup: tup[0])
        to_clipboard = ""
    ...
    def save_size(self):
        ...
        self.settings["width"], self.settings["height"] = self.windowWidth(
        ), self.windowHeight()
    ...
    def openMenuTree(self, position):
        ...
        if not selectedItems:
            ...
            copyToClipboard = menu.addAction(_translate(
                "Tree context menu", "Copy to clipboard"))
            ...
            editAction = menu.addAction(
                _translate("Tree context menu", "Edit..."))
            ...
            for item in self.treeWidget.selectedItems():
                if item.childCount():
                    ...
                    break
            menu.exec_(self.treeWidget.viewport().mapToGlobal(
                position)+QtCore.QPoint(3, 0))
        else:
    ...
    def openMenuTable(self, position):
        ...
        if n > 0:
            menu = QtWidgets.QMenu()
            copyToClipboard = menu.addAction(_translate(
                "Table context menu", "Copy to clipboard"))
            copyToClipboard.setShortcut(QtGui.QKeySequence.Copy)
            copyToClipboard.triggered.connect(self.copyToClipboardFromTable)
            editAction = menu.addAction(
                _translate("Table context menu", "Edit..."))
            editAction.triggered.connect(self.editExerciceTableWidget)
            moveUp = menu.addAction(_translate(
                "Table context menu", "Move up"))
            moveUp.triggered.connect(self.gouptable)
            moveDown = menu.addAction(_translate(
                "Table context menu", "Move down"))
            moveDown.triggered.connect(self.godowntable)
            shuffle = menu.addAction(_translate(
                "Table context menu", "Shuffle"))
            ...
            if self.settings['AMC'] == 'True':
                AMC = menu.addMenu(_translate(
                    "AMC-Menu", "Set element name (AMC)"))
                action = AMC.addAction(_translate(
                    "AMC-Menu", "New element..."))
            ...
            menu.exec_(self.tableWidget.viewport().mapToGlobal(
                position)+QtCore.QPoint(3, 0))
    ...
    def newElement(self):
        ...
        ui_newelt.label.setText(_translate(
            "AMC-Menu", "Enter the new element name"))
    ...
    def initiatePdf(self):
        ...
        self.pdfwidget = PDFWidget(
            None, parent=self.pdfWidgetContainer, width=w-30)
    ...
    def setupUi2(self, Form):
        ...
        if self.settings['AMC'] == 'True':
            ...
            self.tableWidget.setHorizontalHeaderLabels(
                [_translate('Form', 'Exercise'), _translate('Form', 'Element (AMC)')])
        # lineEdit
        self.lineEdit.setText(self.settings["tex_path"])
        ...
        l1 = [int(self.settings["little_splitter_s1"]),
              int(self.settings["little_splitter_s2"])]
        l2 = [int(self.settings["big_splitter_s1"]),
              int(self.settings["big_splitter_s2"])]
        Form.resize(l, w)
        self.big_splitter.setSizes(l2)
        self.little_splitter.setSizes(l1)
        # Table sizes
        self.tableWidget.horizontalHeader().setStretchLastSection(True)
        self.tableWidget.setColumnWidth(0, int(.6*l1[1]))
        self.initiatePdf()
